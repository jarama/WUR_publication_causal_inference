...
results[, `:=`(
  ...
  incl_vars = factor(incl_vars, levels = unique(results$incl_vars)),
...
ggplot(
  ...
  geom_violin() +
  geom_point(
    data = dedup, aes(x = incl_vars, y = total_effect, color = "blue"),
    size = 3, inherit.aes = FALSE
  ) +
  geom_point(
    data = dedup, aes(x = incl_vars, y = direct_effect, color = "green"),
    size = 2, inherit.aes = FALSE
  ) +
  geom_point(
    data = dedup, aes(x = incl_vars, y = indirect_effect, color = "red"),
  ...
  scale_color_manual(
    ...
    labels = c("total", "direct", "indirect")
  )

# less interesting, given analytic solution for effect n and Y_sd on coef_T
if (FALSE) {
  # incl_vars,n,Y_sd
  dedup <- unique(
    results[
  ...
  ggplot(
    ...
    geom_violin() +
    geom_point(
      data = dedup, aes(x = incl_vars, y = direct_effect),
      color = "green", size = 3, inherit.aes = FALSE
    ) +
    geom_point(
      data = dedup, aes(x = incl_vars, y = indirect_effect),
      color = "red", size = 3, inherit.aes = FALSE
    ) +
    geom_point(
      data = dedup, aes(x = incl_vars, y = total_effect),
  ...
  ggplot(
    ...
    geom_violin() +
    geom_point(
      data = dedup, aes(x = n, y = direct_effect),
      color = "green", size = 3, inherit.aes = FALSE
    ) +
    geom_point(
      data = dedup, aes(x = n, y = indirect_effect),
      color = "red", size = 3, inherit.aes = FALSE
    ) +
    geom_point(
      data = dedup, aes(x = n, y = total_effect),
      color = "yellow", size = 3, inherit.aes = FALSE
    ) +
    facet_grid(beta_1 ~ beta_0,
      labeller = label_both
  ...
  ggplot(
    ...
    geom_violin() +
    geom_point(
      data = dedup, aes(x = Y_sd, y = direct_effect),
      color = "green", size = 3, inherit.aes = FALSE
    ) +
    geom_point(
      data = dedup, aes(x = Y_sd, y = indirect_effect),
      color = "red", size = 3, inherit.aes = FALSE
    ) +
    geom_point(
      data = dedup, aes(x = Y_sd, y = total_effect),
      color = "yellow", size = 3, inherit.aes = FALSE
    ) +
    facet_grid(beta_1 ~ beta_0,
      labeller = label_both
